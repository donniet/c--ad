...
constexpr unit_id_type zero_unit_id =               { 0, 0 };
...
template< arithmetic T >
struct base_unit< zero_unit_id, T >
{
    static constexpr bool is_unit = true;
    using scalar_type = T;
    static constexpr unit_id_type unit_id = scalar_unit_id;
    ...
    constexpr base_unit&
    operator =( base_unit const& other )
    { return *this; }
    ...
    template< unit_id_type Id, arithmetic U >
    consteval operator base_unit< Id, U >() const
    { return base_unit< Id, T >{ U{ 0 }}; }

    consteval operator scalar_type() const
    { return scalar_type{ 0 }; }
    consteval operator bool() const
    { return false; }

    constexpr base_unit() = default;
    constexpr base_unit( base_unit const& ) = default;
    constexpr base_unit( exact_zero ) { }

    consteval scalar_type
    get_value() const
    { return 0; }
};

template< >
struct base_unit< scalar_unit_id, bool >
{
    static constexpr bool is_unit = true;
...
///
using Zero = base_unit< zero_unit_id, int >;
...
constexpr bool 
operator ==( LeftU const& left, RightU const& right )
{ return left.get_value() == right.get_value(); }

template< typename U, typename RightT >
requires( not unit< RightT > and requires( U l, RightT r) { l == r; } )
constexpr bool 
operator ==( base_unit< scalar_unit_id, U > const& left, RightT const& right )
{ return left.get_value() == right; }
...
template< unit LeftU, unit RightU >
requires( unit_traits< LeftU >::unit_id == unit_traits< RightU >::unit_id )
constexpr bool operator !=( LeftU const& left, RightU const& right )
{ return left.get_value() != right.get_value(); }
...
template< units::unit_id_type uid, arithmetic T >
